	...
	public class TOS_Patch : ModulePatch
	{
		...
		private static IEnumerator Display()
		{
			...
			if (!FikaPlugin.AcceptedTOS.Value)
			{
				...
				Singleton<PreloaderUI>.Instance.ShowFikaMessage("FIKA AND MTC-C", str_1_d, ErrorScreen.EButtonType.QuitButton, 15f,
			...
			else
			{
				...
				Singleton<PreloaderUI>.Instance.ShowFikaMessage("FIKA AND MTC-C", str_2_d, ErrorScreen.EButtonType.OkButton, 0f,
					null,
					null);
			}
		}
